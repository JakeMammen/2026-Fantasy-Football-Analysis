...
ggplot(avg_exp_fpWR_df, aes(x = td, y = tds_exp)) +
  ...
  labs(
    title = "2024 NFL Wide Receivers: Actual vs. Expected Touchdowns",
    x = "Actual Touchdowns",
    y = "Expected Touchdowns",
    caption = "Data: nflfastr and nflreadr | By: Jake Mammen | @FantasySPack"
  ) +
  geom_label(x = 5, y = 12, label = "Postive TD Regression Candidates", fill = "purple", color = "white", label.size = 0.5 ) +
  geom_label(x = 14, y = 4, label = "Negative TD Regression Candidates", fill = "purple", color = "white", label.size = 0.5,) +
  theme_minimal() +
  theme(
...
ggplot(avg_exp_fpTE_df, aes(x = td, y = tds_exp)) +
  geom_image(aes(image = team_logo_espn), size = 0.05) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "purple") +
  geom_text_repel(aes(label = player_name), box.padding = 0.5, max.overlaps = 22) +
  labs(
    title = "2024 NFL Tight Ends: Actual vs. Expected Touchdowns",
    x = "Actual Touchdowns",
    y = "Expected Touchdowns",
    caption = "Data: nflfastr and nflreadr | By: Jake Mammen | @FantasySPack"
  ) +
...
# Load player stats for 2024 regular season
pbp <- nflreadr::load_player_stats(seasons = 2024, stat_type = "offense") %>%
  filter(season_type == "REG", week >= 1, week <= 18)

# Load roster data to get player positions
roster <- nflreadr::load_rosters(2024) %>%
  dplyr::select(gsis_id, position, full_name) %>%
  distinct()

# Process passing/receiving stats
passWRTE_df <- pbp %>%
  filter(position %in% c("WR", "TE")) %>%  # Filter for WRs and TEs early
  dplyr::select(
    season, week, team = recent_team, player_id, player_name, 
    position, receptions, targets, passing_yards, passing_tds, receiving_yards, receiving_tds, 
    receiving_fumbles_lost, receiving_2pt_conversions
  ) %>%
  mutate(
...
# Process rushing stats for receivers
rushWRTE_df <- pbp %>%
  filter(position %in% c("WR", "TE"), rushing_yards > 0 | rushing_tds > 0) %>%
  dplyr::select(
    season, week, team = recent_team, player_id, player_name, 
    position, rushing_yards, rushing_tds, rushing_fumbles_lost, rushing_2pt_conversions
  ) %>%
...
# Load expected fantasy points for the 2024 season, weeks 1-18
ff_opportunity_WRTE <- load_ff_opportunity(seasons = 2024, stat_type = "weekly", model_version = "latest")

# Filter for wide receivers (WR) and select relevant columns
receivers_tightends <- ff_opportunity_WRTE %>%
...
# Aggregate expected points by receiver across weeks 1-18
receiver_tightends_totals <- receivers_tightends %>%
  filter(week %in% 1:18) %>%
  group_by(player_id, full_name) %>%
  summarise(
    games_played = n(),
...
avg_fpWRTE_df <- passWRTE_df %>%
  ...
  filter(position %in% c("WR", "TE"), games >= 8) %>%
  dplyr::select(team, player_name, games, targets, catches, yards, td, fumbles, PPR_pts, rec_exp, yds_exp, tds_exp, total_expected_points, forp)
...
avg_exp_fpWRTE_df <- avg_fpWRTE_df %>%
  mutate(fp_game = PPR_pts / games) %>%
  filter(games >= 10 & fp_game >= 10) %>%
  left_join(teams_colors_logos, by = c("team" = "team_abbr")) 

ggplot(avg_exp_fpWRTE_df, aes(x = td, y = tds_exp)) +
  geom_image(aes(image = team_logo_espn), size = 0.05) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "purple") +
  geom_text_repel(aes(label = player_name), box.padding = 0.5, max.overlaps = 22) +
  labs(
    title = "2024 NFL WRs & TEs: Actual vs. Expected Touchdowns",
    x = "Actual Touchdowns",
    y = "Expected Touchdowns",
    caption = "Data: nflfastr and nflreadr | By: Jake Mammen | @FantasySPack"
  ) +
...
# Process passing/receiving stats
passRB_df <- pbp %>%
  filter(position %in% c("RB")) %>%  # Filter for WRs early
  dplyr::select(
    season, week, team = recent_team, player_id, player_name, 
    position, receptions, targets, passing_yards, passing_tds, receiving_yards, receiving_tds, 
    receiving_fumbles_lost, receiving_2pt_conversions
...
# Load expected fantasy points for the 2024 season, weeks 1-18
ff_opportunity <- load_ff_opportunity(seasons = 2024, stat_type = "weekly", model_version = "latest")
...
# Create the gt table
fp_RB2024 <- avg_fpRB_df %>%
  arrange(-PPR_pts) %>%
  dplyr::slice(1:80) %>%
  mutate(Rank = paste0('#', row_number())) %>%
  gt() %>%
  tab_header(title = md('**2024 Actual vs. Expected PPR Fantasy Points Running Backs**')) %>%
  cols_move_to_start(columns = vars(Rank)) %>%
  cols_label(
    games = 'GP',
    player_name = '',
  ...
  data_color(
    columns = vars(PPR_pts, total_expected_points),
    colors = scales::col_numeric(palette = c('grey97', '#E03FD8'), domain = c(8, 405)),
    autocolor_text = FALSE
  ) %>%
  data_color(
    columns = vars(forp),
    colors = scales::col_numeric(palette = c('#FF4040', '#FFFFFF', '#40C040'), domain = c(-50, 0, 90)),
    autocolor_text = FALSE
  ) %>%
  text_transform(
    locations = cells_body(vars(team)),
  ...
  tab_options(
    ...
    table.background.color = '#F2F2F2',
    table.border.bottom.color = 'transparent',
    source_notes.background.color = 'purple',
    row.striping.background_color = '#FFFFFF',
    row.striping.include_table_body = TRUE
  ) %>%
  tab_source_note(
    source_note = html(
      "<div style='display: flex; align-items: center; justify-content: space-between;'>
         <div style='font-size: 12px;'>
           <b>Data:</b> nflreadr | <b>Source:</b> Credit: Anthony Reinhard, 2020, Open Source Football |
           <b>Created by:</b> Jake Mammen | @FantasySPack & @jakemammenwx
         </div>
         <img src='/Users/jakemammen/Library/CloudStorage/OneDrive-Personal/Desktop/Masters Program/Fantasy_Football/IMG_4101.jpg' style='height: 30px; margin-left: 10px;'>
       </div>"
    )
  )

gtsave(fp_RB2024, "exp_fp_RB2024.png")
...
passQB_df <- pbp %>%
  ...
  dplyr::select(
    ...
    interceptions, sack_fumbles_lost, rushing_fumbles_lost, passing_2pt_conversions
  ) %>%
  mutate(
    receiving_yards = case_when(
      player_id == "00-0034857" & week == 13 ~ receiving_yards + 7,  # Add 7 receiving yards for J. Allen in week 13
      TRUE ~ receiving_yards
    ),
    receiving_tds = case_when(
      player_id == "00-0034857" & week == 13 ~ receiving_tds + 1,  # Add 1 receiving TD for J. Allen in week 13
      TRUE ~ receiving_tds
    ),
    sack_fumbles_penalty = ifelse(sack_fumbles_lost >= 1, -2 * sack_fumbles_lost, 0),
    int_penalty = ifelse(interceptions >= 1, -2 * interceptions, 0),
...
# Create the gt table
fp_QB2024 <- avg_fpQB_df %>%
  arrange(-PPR_pts) %>%
  dplyr::slice(1:39) %>%  # Adjust for typical number of starting QBs
  mutate(Rank = paste0('#', row_number())) %>%
  gt() %>%
  tab_header(title = md('**2024 Actual vs. Expected PPR Fantasy Points Quarterbacks**')) %>%
  cols_move_to_start(columns = vars(Rank)) %>%
  cols_label(
    games = 'GP',
    player_name = '',
  ...
  data_color(
    columns = vars(forp),
    colors = scales::col_numeric(palette = c('#FF4040', '#FFFFFF', '#40C040'), domain = c(-54, 0, 95)),
    autocolor_text = FALSE
  ) %>%
  text_transform(
    locations = cells_body(vars(team)),
...
gtsave(fp_QB2024, "exp_fp_QB2024.png")
...
ggplot(avg_exp_fpQB_df, aes(x = td, y = tds_exp)) +
  geom_image(aes(image = team_logo_espn), size = 0.05) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "purple") +
  geom_text_repel(aes(label = player_name), box.padding = 0.5, max.overlaps = 22) +
  labs(
    title = "2024 NFL Quarterbacks: Actual vs. Expected Touchdowns",
    x = "Actual Touchdowns",
    y = "Expected Touchdowns",
    caption = "Data: nflfastr and nflreadr | By: Jake Mammen | @FantasySPack"
  ) +
